...
function reset(r){clearTimeout(r.timer);r.phase='lobby';r.target=null;r.players.forEach(p=>p.ready=false);r.votes={};emit(r)}
function start(r){if(r.phase!=='lobby'||r.players.length<2)return;if(r.players.length<r.maxPlayers&&!r.players.every(p=>r.votes[p.id]))return;if(r.players.length===r.maxPlayers&&!r.players.every(p=>p.ready))return;r.phase='playing';r.players.forEach(p=>p.ready=false);emit(r);r.timer=setTimeout(()=>{if(!rooms.has(r.id)||r.phase!=='playing')return;r.target={x:10+Math.random()*80,y:15+Math.random()*70,color:['red','green','gold','violet','blue'][Math.floor(Math.random()*5)]};r.targetAt=Date.now();io.to(r.id).emit('target:show',r.target)},1800+Math.random()*5000)}
...
io.on('connection',s=>{
 ...
 s.on('target:click',()=>{const p=players.get(s.id),r=p&&rooms.get(p.roomId);if(!r||r.phase!=='playing'||!r.target||r.winner)return;const me=r.players.find(x=>x.id===s.id);r.winner=s.id;r.phase='result';clearTimeout(r.timer);const pot=+(r.stake*r.players.length).toFixed(2),reaction=Date.now()-r.targetAt;me.credits+=pot;io.to(r.id).emit('game:result',{winnerId:s.id,winnerName:me.name,pot,reaction});setTimeout(()=>{if(rooms.has(r.id))reset(r)},6000)});
